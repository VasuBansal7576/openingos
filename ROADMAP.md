...
4. [ ] **R1 / C1 / U1, parallel features.** After accepted F1 contracts land, dispatch research, communication and workbench packages with exclusive file ownership.
   ...
   - [ ] Obtain the final fixed-head Astra review after pushing combined repair code checkpoint `b5db30e`; Astra rejected reviewed head `d582d0c`, and all five findings are now owner-repaired and integrated.
5. [ ] **E1, integrated behavior.** Build recovery, change impact and equipment/service flows against the integrated research and communication modules.
   ...
   - [ ] Finish hosted/live recovery evidence; the controlled integration, P01 intake and Astra repairs pass locally through code checkpoint `b5db30e`, but hosted provider evidence and fixed-head Astra acceptance remain open.
...
- [x] Route and integrate all five Astra repairs through combined code checkpoint `b5db30e`. PR #15 head `e4a9e51` derives and enforces the server-owned inbox and rejects changed same-key replay; PR #16 head `d3648f5` uses a deterministic controlled clock; P01 integration fixes money/source rendering. The full combined suite passes with 23 repository, 77 proof, 15 eval, 338 F1, 155 browser, 91 application, 18 Jev, 32 communication and 509 direct tests, plus both strict typechecks and the production build.
- [x] Inspect the actual unconfigured runtime through Chrome DevTools Protocol at 1440 by 1200 and exact 390 by 844 viewports. Both document widths equal their viewports with zero overflow offenders, and the rendered shell matches the selected sage-desk, paper-card, serif and yellow-decision visual system. This is controlled local evidence, not populated or live-provider success.
- [ ] Obtain a fresh separate GPT-6 Astra ACCEPT verdict on the pushed descendant of code checkpoint `b5db30e` and route any new findings before merge.
