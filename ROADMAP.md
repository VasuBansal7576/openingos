...
- [ ] Obtain a separate GPT-6 Astra `ACCEPT` verdict on the final exact combined head before merge. Astra rejected `bd3fe6a` with six findings; mailbox privacy, anonymous auth, strict quote binding, exact budget parsing, app-CI parser coverage and visual contrast/fidelity repairs are now integrated through `b47887e`. A new fixed-head review remains mandatory.
...
- [x] Safely remove the completed E13, E15, E17 and E18 task worktrees after clean-status and remote-preservation checks. E16 remains only until its merged work, temporary QA artifacts and stopped browser processes are settled.
