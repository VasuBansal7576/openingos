...
4. [ ] **R1 / C1 / U1, parallel features.** After accepted F1 contracts land, dispatch research, communication and workbench packages with exclusive file ownership.
   ...
   - [x] Obtain a fresh fixed-head Astra review after pushing the combined descendant. Astra reviewed exact PR #14 head `1b2850c` and returned `CHANGES REQUIRED`: three prior findings are repaired, F2 comparison and F4 source retrieval remain open, and F6 through F9 add authority, budget, evidence-log and contrast repairs.
5. [ ] **E1, integrated behavior.** Build recovery, change impact and equipment/service flows against the integrated research and communication modules.
   ...
   - [ ] Finish the six exact-head Astra repairs and hosted/live recovery evidence. The controlled integration suites pass at `1b2850c`, but that head is rejected for F2, F4 and F6 through F9; hosted provider evidence remains separately open.
...
- [x] E8 correctness, the selected purchasing-desk fidelity rebuild, E11 and the repaired E12/F1 authority packages are integrated locally at `220165a` without deleting E8 behavior tests. The exact merge candidate passes `bun run test`, strict TypeScript and the production build. Controlled Chrome DevTools inspection at 1440×1200 and 390×844 reports zero horizontal overflow and zero console errors for the honest unconfigured state; no fixture vendor or provider result is shown. Full populated-flow visual acceptance remains open until Playwright or Orca Computer Use can run against an authorized backend projection.
...
- [ ] Complete the five repairs from Astra's rejection of `d582d0c`. Exact head `1b2850c` independently verifies the sender-binding, changed-replay and deterministic-clock repairs, while F2 money comparison and F4 original-source retrieval remain open. The full combined suite still passes with 23 repository, 77 proof, 15 eval, 338 F1, 155 browser, 91 application, 18 Jev, 32 communication and 509 direct tests, plus both strict typechecks and the production build.
- [x] Inspect the actual unconfigured runtime through Chrome DevTools Protocol at 1440 by 1200 and exact 390 by 844 viewports. Both document widths equal their viewports with zero overflow offenders, but this check did not establish prototype fidelity: the production entry still replaces the accepted landing flow with a dominant backend-configuration error. This is controlled local layout evidence only.
...
- [ ] Integrate F2, F4 and F6 through F9, rerun exact-head CI and obtain a separate GPT-6 Astra `ACCEPT` verdict on the replacement commit before merge.
...
- [ ] PRs #7 through #13 remain individually unmerged despite their code being present in the combined branch, because Greptile supplied only quota notices. Close through the fixed combined PR only after exact-head CI and fresh Astra acceptance.
