...
It records controlled evidence through exact combined checkpoint `b47887e` on September 21, 2026.
That checkpoint passes the complete local matrix and controlled desktop/mobile browser QA; latest-head GitHub checks and independent Astra acceptance remain merge gates.
...
| P-05 | live-unverified | The application now stores the canonical owner mailbox envelope only in protected evidence and resolves it server-side at dispatch, while public preparation returns a draft ID and redacted preview. A real owner-only AgentMail send/reply and resulting project update remain unverified. |
...
| P-12 | partial-controlled | PR #14 preserves original history, derives substitutes with explanation, fences stale approval and keeps rejection available after a disruption; fixed-head review remains open. |
...
| S-20 | partial-controlled | At combined checkpoint `b47887e`, every connection state remains inside the selected workbench shell, last-known reconnect state blocks mutations until a fresh projection, and exact 390 × 844 controlled browser inspection has document width equal to viewport width. The controlled backend-generated sample exercises suppliers, evidence, recovery and equipment; hosted reconnect behavior remains unverified. |
...
- Current combined checkpoint: `b47887e`, integrating P01 intake, accepted-prototype fidelity, authenticated isolated sample guest projects, protected mailbox envelopes, real anonymous-auth bootstrap, strict comparison parsing, exact budget handling and the final decision-desk responsive pass. The full local matrix, both strict TypeScript checks, production build and controlled desktop/mobile browser QA pass; latest-head CI and separate fixed-head Astra acceptance remain open.
