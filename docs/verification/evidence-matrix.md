...
| S-20 | partial-controlled | At PR #14 head `ed0979c`, every connection state remains inside the selected workbench shell, last-known reconnect state blocks mutations until a fresh projection, and exact 390×844 controlled browser inspection has `clientWidth = scrollWidth = 390`. Hosted reconnect behavior remains unverified. |
...
- Combined repair code checkpoint: `b5db30e`, integrating P01 intake and three of the five findings from Astra's rejection of `d582d0c`: sender binding, exact replay conflicts and deterministic expiry tests. Exact pushed descendant `1b2850c` passes the full local matrix, both strict TypeScript checks, the production build and all nine configured GitHub checks, but an independent Astra review still reproduces F2 money comparison and F4 original-source retrieval and adds F6 through F9. Acceptance remains open.
