...
It records controlled evidence through exact combined checkpoint `9957b2d` on September 21, 2026.
That checkpoint passes the complete local matrix and all 18 configured GitHub checks; independent Astra acceptance remains open.
...
| P-21 | partial-controlled | The selected purchasing-desk system renders real projections plus honest unconfigured, loading, empty, error and reconnecting states through combined checkpoint `9957b2d`. Controlled Chrome DevTools Protocol inspection covers static landing parity plus a sample generated by the real sample mutation and ordinary projection at 1440×1100 and exact 390×844, with no horizontal overflow and passing supplier, evidence-dialog, keyboard and equipment interactions. Hosted authenticated verification remains open. |
...
| S-20 | partial-controlled | At PR #14 head `9957b2d`, every connection state remains inside the selected workbench shell, last-known reconnect state blocks mutations until a fresh projection, and exact 390×844 controlled browser inspection has `clientWidth = scrollWidth = 390`. The controlled backend-generated sample exercises suppliers, evidence and equipment; hosted reconnect behavior remains unverified. |
...
- Combined replacement checkpoint: `9957b2d`, integrating P01 intake, accepted-prototype fidelity, authoritative money/source repairs, temporary-authority and budget fixes, and authenticated isolated sample guest projects. The full local matrix, both strict TypeScript checks, production build and all 18 configured GitHub checks pass; separate fixed-head Astra acceptance remains open.
