...
It records evidence available at coordinator checkpoint `6432e98` on September 20, 2026.
...
| P-15 | not-started | Cross-organization and restricted-project direct-call tests belong to F1. |
| P-16 | not-started | Guest/private record isolation belongs to F1 and published-origin verification. |
...
| P-20 | not-started | Contributor/approver cross-project authority needs F1 and workbench implementation. |
...
| D-07 | not-started | Bounded communication briefs and changed-recipient review need F1/C1. |
...
| D-14 | partial-controlled | Executor authorization, expiry, revocation, origin and operation denials pass controlled review; full backend capability/grant path remains. |
| D-15 | not-started | Changed-term re-evaluation without history overwrite needs E1/U1. |
| D-16 | not-started | Shared concurrent allowance reservations belong to F1. |
| D-17 | partial-controlled | The foundation UI demonstrates a controlled unrelated-request refusal; backend enforcement, mixed requests and injection resistance belong to F1. |
...
| S-04 | not-started | F1 direct-API identity isolation matrix. |
| S-05 | not-started | F1/C1 request idempotency and single-send concurrency. |
| S-06 | not-started | F1/C1 pre-claim revocation/expiry/change/reply/cancel races. |
| S-07 | not-started | C1 late-send versus cancellation state separation. |
| S-08 | not-started | F1 shared allowance and unknown-charge reservation concurrency. |
| S-09 | not-started | F1/C1 crash/replay reconciliation without resend. |
| S-10 | not-started | F1 backend refusal with a valid parallel research job. |
...
| S-22 | not-started | F1/C1 owner-recipient enforcement across API, model, retry and browser boundaries. |
...
| J-03 | partial-controlled | Controlled adapter tests cover retry classes, timeout, cancellation and attempt bounds; shared application allowance concurrency belongs to F1. |
| J-04 | partial-controlled | Controlled executor and adapter freshness tests fence late/stale authority; recipient and full grant integration belong to F1/C1. |
