...
### 2026-09-22 - OpenAI generation path exercised via owner Codex subscription transport

The owner declined to purchase an OpenAI Platform API key, so a demo transport was built: `scripts/codex-subscription-proxy.mjs`, a localhost-only Node server that accepts the exact Responses API envelope the product adapter already sends, executes the owner's authenticated `codex exec` (ChatGPT/Codex subscription, configured model `gpt-6-astra`) in a serialized 300-second-bounded subprocess, and repackages the result as a Responses-shaped reply. The proxy is explicitly labeled demo transport, not product code; it requires a shared bearer secret, binds to 127.0.0.1, and its usage numbers are byte-derived synthetics, not provider-metered tokens.

To reach it, `convex/models/openai.ts` gained three bounded env overrides: `OPENAI_ENDPOINT` (HTTPS-only; a present-but-invalid value now refuses with `invalid-endpoint-config` before any fetch rather than falling back to api.openai.com), `OPENAI_MODEL` (the recorded and response-validated model identity), and `OPENAI_TIMEOUT_MS`. `convex/negotiation/orchestrator.ts` now checks drafts against the runtime-pinned model, and `convex/convex.config.ts` declares every env var the backend reads so regenerated `Env` types stay consistent.

A SWE-2 High Orca reviewer first REJECTED the change: an invalid endpoint override silently fell back to api.openai.com, which could have egressed the workload and bearer secret to the wrong target. The fail-closed fix (`aef7ea4`) was re-reviewed and ACCEPTed at `7ed6c7e`; PR #39 merged to `main` at `6ba43c7` with all checks green and was deployed to prod `enchanted-caiman-248`.

A real F1 chain then ran on prod through public product mutations under the Amsterdam project's anonymous identity: grant issued (`mh7b858c3s2f2q4g6evfxen2598excae`), job started, reservation held, operation `nh7anpgvm7w53d4x3zb5y5qf798ex9bd` created with the workload SHA-256 bound on grant, job and operation rows, then the internal `models/openai:generate` action claimed it and called the configured endpoint — a temporary cloudflared tunnel to the local proxy, which ran `codex exec` against the owner's subscription.

Result: attempt `jh7fytkpw2q61yz6x3tt2rc9ts8ew34e` and the operation both record `observedSuccess`; outcome `completed`, model `gpt-6-astra`, one attempt, 29.3 s latency. The structured commercialExtraction output names Stooker Specialty Coffee (Westhavenweg 62F, Amsterdam) with real observed price signals, services and location extracted from the Firecrawl-sourced page content — evidence saved under `openingos-e2e-proof/codex-sub-run/`.

Honesty boundary: this is an OpenAI model generating real structured output inside the product's real claim/fence/record machinery, but the transport is the owner's ChatGPT/Codex subscription via a local proxy over a temporary tunnel — not an OpenAI Platform API key and not durable production infrastructure. The OpenAI path stops working when the laptop, proxy or tunnel stops. Supplier drafts, Jev pricing calls through the same machinery, and the AgentMail round trip remain unexercised.
