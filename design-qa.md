...
- Automated verification, all passing on this checkout:
  - `bun test app/tests/application.test.ts`: 13 pass
  ...
  - `bun test app/tests/user-path.test.ts`: 14 pass
  ...
  - `bun test app/tests/workbench.test.ts`: 61 pass
  ...
  - `bun run test:app`: 117 Bun application tests plus 18 Jev Vitest
    tests pass.
  - `bun run test:direct`: 527 direct Convex tests pass, including the
...
final result: static landing fidelity and the controlled backend-generated
sample workbench passed local desktop/mobile browser comparison and interaction
verification. Full design QA remains pending hosted authenticated verification
and live provider operations. No client-side sample fixture was introduced.
