...
function ProvenancePill({ mode, ownerAuthoredTerms = false }: { readonly mode: "controlled" | "recorded" | "live"; readonly ownerAuthoredTerms?: boolean }) {
  return (
    ...
    <>
      ...
      <header className="wb-header">
        ...
        <div className="wb-project-picker" aria-label="Current project">
          ...
          <span>{project.region} · {project.currency}</span>
        </div>
...
function RequirementCard({ snapshot, requirement }: { readonly snapshot: WorkbenchSnapshot; readonly requirement: WorkbenchSnapshot["requirements"][number] }) {
  const delivered = snapshot.deliveredQuantityByRequirement[requirement.id] ?? "0";
...
function QuoteSummary({ quote, currency }: { readonly quote: WorkbenchQuote | null; readonly currency: string }) {
...
function OfferCard({ offer, snapshot, selected, onReview }: { readonly offer: WorkbenchOffer; readonly snapshot: WorkbenchSnapshot; readonly selected: boolean; readonly onReview: () => void }) {
  ...
  return (
    <article className={`wb-offer-card ${selected ? "selected" : ""} ${isIncompatible ? "incompatible" : ""}`}>
      <div className="wb-offer-header"><div className="wb-vendor-monogram">{offer.vendor.name.slice(0, 1).toUpperCase()}</div><div><h3>{offer.vendor.name}</h3><p>{offer.productModel} · {offer.variant}</p></div><ProvenancePill mode={offer.provenance} ownerAuthoredTerms={offer.ownerAuthoredTerms} /></div>
      ...
      <div className="wb-offer-foot"><span>{offer.quote?.validUntil ? `Valid through ${formatDate(offer.quote.validUntil)}` : "Validity not confirmed"}</span><ActionButton kind="secondary" disabled={isIncompatible} onClick={onReview} title={isIncompatible ? "An incompatible offer cannot be selected" : undefined}>{selected ? "Selected offer" : "Review quote"}<Icon name="arrow" size={15} /></ActionButton></div>
...
function ActivityList({ items, compact = false }: { readonly items: readonly WorkbenchActivityItem[]; readonly compact?: boolean }) {
  ...
  return <div className={`wb-activity-list ${compact ? "compact" : ""}`}>{items.map((item) => <div className="wb-activity-item" key={item.id}><span className={`wb-activity-icon wb-activity-${item.state}`}><Icon name={item.state === "failed" ? "warning" : item.state === "pending" ? "sync" : item.state === "unknown" ? "dots" : "check"} size={14} /></span><div><strong>{item.summary}</strong><p>{item.actorLabel} · {formatDate(item.occurredAt)}</p></div><Pill tone={item.state === "failed" ? "danger" : item.state === "unknown" ? "warning" : item.state === "pending" ? "neutral" : "success"}>{formatStateLabel(item.state)}</Pill></div>)}</div>;
}
...
function SelectionPanel({ offer, snapshot, onClose, onAction, onMessage }: { readonly offer: WorkbenchOffer; readonly snapshot: WorkbenchSnapshot; readonly onClose: () => void; readonly onAction?: WorkbenchViewProps["onAction"]; readonly onMessage: (message: string) => void }) {
  ...
  const canSelect = snapshot.access.capabilities.canApprove && offer.compatibility === "pass" && quote !== null && quote.comparableTotalMinorUnits !== null && !quote.superseded;
  const choose = async () => {
  ...
  return <div className="wb-overlay"><section className="wb-selection-panel" role="dialog" aria-modal="true" aria-labelledby="selection-title"><div className="wb-panel-head"><div><span className="wb-eyebrow">DECISION REVIEW</span><h2 id="selection-title">{offer.vendor.name} · quote v{quote?.version ?? "unknown"}</h2></div><button className="wb-icon-button" type="button" onClick={onClose} aria-label="Close decision review"><Icon name="close" size={18} /></button></div><div className="wb-selection-content"><div className="wb-selection-brand"><span className="wb-vendor-monogram large">{offer.vendor.name.slice(0, 1).toUpperCase()}</span><div><h3>{offer.productModel}</h3><p>{offer.variant} · {offer.vendor.regions.join(", ") || "Service region unknown"}</p></div><ProvenancePill mode={offer.provenance} ownerAuthoredTerms={offer.ownerAuthoredTerms} /></div><QuoteSummary quote={quote} currency={snapshot.project.currency} /><div className="wb-decision-explanation"><Icon name="shield" size={19} /><div><strong>Selection is not an order.</strong><p>This exact quote would change the selected forecast only. It does not create a commitment, place an order or take payment.</p></div></div>{!snapshot.access.capabilities.canApprove ? <div className="wb-inline-warning"><Icon name="lock" size={16} /> Your role cannot approve this decision. An authorized approver must review it.</div> : null}{quote?.superseded ? <div className="wb-inline-warning"><Icon name="warning" size={16} /> This quote version is superseded. Refresh the projection before approving.</div> : null}<ActionButton disabled={!canSelect} onClick={() => { void choose(); }} title={!onAction ? "No server action is attached" : !canSelect ? "A complete, compatible, current quote is required" : undefined}>Select exact quote <Icon name="check" size={16} /></ActionButton><p className="wb-micro">No order will be placed. No payment will be taken.</p></div></section></div>;
}
...
function InboxView({ snapshot, onAction, onMessage }: { readonly snapshot: WorkbenchSnapshot; readonly onAction?: WorkbenchViewProps["onAction"]; readonly onMessage: (message: string) => void }) {
  ...
  const approve = async (decisionId: string) => {
    if (!snapshot.access.capabilities.canApprove || !onAction) { onMessage("Approval is blocked until the server confirms an approver and a current action route."); return; }
...
function JobRow({ job, snapshot, onAction, onMessage }: { readonly job: WorkbenchSnapshot["jobs"][number]; readonly snapshot: WorkbenchSnapshot; readonly onAction?: WorkbenchViewProps["onAction"]; readonly onMessage: (message: string) => void }) {
  ...
  const run = async (type: "retryJob" | "cancelJob") => {
    if (!onAction || (type === "retryJob" && !snapshot.access.capabilities.canResearch) || (type === "cancelJob" && !snapshot.access.capabilities.canResolveRisk)) { onMessage("This action is not authorized in the current project role or connection state."); return; }
  ...
  return <article className="wb-job-row"><div className="wb-job-state"><StatusDot state={job.delivery} /><div><strong>{formatStateLabel(job.kind)} · {deliveryLabel(job.delivery)}</strong><p>{job.summary}</p></div></div><div className="wb-job-progress"><span>{job.progress === null ? "Progress unknown" : `${Math.round(job.progress * 100)}%`}</span><div className="wb-progress-track"><span style={{ width: `${job.progress === null ? 0 : Math.max(0, Math.min(100, job.progress * 100))}%` }} /></div></div><div className="wb-job-meta"><span>Attempt {job.attempts}</span><span>Updated {formatDate(job.updatedAt)}</span></div><div className="wb-job-actions">{canRetry ? <ActionButton kind="secondary" disabled={!snapshot.access.capabilities.canResearch} onClick={() => { void run("retryJob"); }}>Retry bounded branch</ActionButton> : null}{canCancel ? <ActionButton kind="text" disabled={!snapshot.access.capabilities.canResolveRisk} onClick={() => { void run("cancelJob"); }}>Cancel</ActionButton> : null}</div>{job.failureCode ? <p className="wb-card-footnote"><Icon name="warning" size={13} /> {job.failureCode} · completed results remain retained.</p> : null}</article>;
}
