...
function SubstituteCard({ substitute, snapshot, onAction, onMessage }: { readonly substitute: WorkbenchSubstitute; readonly snapshot: WorkbenchSnapshot; readonly onAction?: WorkbenchViewProps["onAction"]; readonly onMessage: (message: string) => void }) {
  ...
  const canDecide = snapshot.access.capabilities.canApprove === true && onAction !== undefined && substitute.state === "pending" && !substitute.basisStale;
  const decide = async (decision: "approved" | "rejected") => {
    ...
    if (substitute.basisStale) { onMessage(`This substitute basis changed (${substitute.basisReason}); renewed authority required. Nothing was sent.`); return; }
    setPending(decision);
    try {
    ...
    <article className="wb-panel wb-substitute-card" aria-label={`Substitute proposal ${substitute.id}`}>
      ...
      {substitute.state === "pending" ? (
        <div className="wb-job-actions">
          <ActionButton kind="secondary" disabled={!canDecide || pending !== null} onClick={() => { void decide("approved"); }} title={!onAction ? "Decisions wait for a connected projection" : snapshot.access.capabilities.canApprove !== true ? "Your role cannot approve substitutes" : substitute.basisStale ? substitute.basisReason : undefined}>{pending === "approved" ? "Approving…" : "Approve substitute"}</ActionButton>
          <ActionButton kind="text" disabled={!canDecide || pending !== null} onClick={() => { void decide("rejected"); }} title={!onAction ? "Decisions wait for a connected projection" : snapshot.access.capabilities.canApprove !== true ? "Your role cannot reject substitutes" : substitute.basisStale ? substitute.basisReason : undefined}>{pending === "rejected" ? "Rejecting…" : "Reject"}</ActionButton>
        </div>
      ) : (
