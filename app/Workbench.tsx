...
function OfferCard({ offer, snapshot, index, selected, onReview, onOpenEvidence }: { readonly offer: WorkbenchOffer; readonly snapshot: WorkbenchSnapshot; readonly index: number; readonly selected: boolean; readonly onReview: () => void; readonly onOpenEvidence?: ((evidence: WorkbenchEvidence) => void) | undefined }) {
...
function ProjectView({ snapshot, onReview, onAction, onLoadMore, onTabChange, onOpenAssistant, onOpenEvidence, onMessage }: { readonly snapshot: WorkbenchSnapshot; readonly onReview: (offer: WorkbenchOffer) => void; readonly onAction?: WorkbenchViewProps["onAction"]; readonly onLoadMore?: (() => void) | undefined; readonly onTabChange: (tab: Tab) => void; readonly onOpenAssistant: () => void; readonly onOpenEvidence: (evidence: WorkbenchEvidence) => void; readonly onMessage: (message: string) => void }) {
  ...
  const comparableEntries: { readonly vendor: string; readonly total: number }[] = [];
  for (const offer of visibleOffers) {
    const total = offer.quote?.comparableTotalMinorUnits ?? null;
    if (offer.vendor !== null && offer.compatibility !== "fail" && total !== null) comparableEntries.push({ vendor: offer.vendor.name, total });
  }
  const lowestComparable = comparableEntries.reduce<{ readonly vendor: string; readonly total: number } | null>((current, entry) => current === null || entry.total < current.total ? entry : current, null);
  const highestComparable = comparableEntries.reduce<{ readonly vendor: string; readonly total: number } | null>((current, entry) => current === null || entry.total > current.total ? entry : current, null);
  const reviewTarget = visibleOffers.find((offer) => offer.id === selected && offer.vendor !== null && offer.compatibility !== "fail")
    ?? visibleOffers.find((offer) => offer.vendor !== null && offer.compatibility !== "fail")
    ?? null;
  const action = async () => {
