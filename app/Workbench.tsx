...
/**
 * Astra F4 source-only research visibility. Collected sources that persisted
 * evidence but correctly created no vendor, candidate, or quote stay visible
 * here with URL, capture/completeness, provenance, and explicit missing
 * commercial facts. They are never rendered as suppliers, products, quotes,
 * offers, or realized savings: no vendor name, no model, no totals, no
 * comparison, and no review/selection action.
 */
function ResearchSourcesSection({ sources, truncated = false }: { readonly sources: readonly WorkbenchResearchSource[]; readonly truncated?: boolean }) {
  if (sources.length === 0) return null;
  return (
    <section className="wb-panel wb-research-sources" aria-label="Research sources without a supplier">
      <div className="wb-panel-heading">
        <div>
          <span className="wb-eyebrow">RESEARCH SOURCES · NO SUPPLIER, QUOTE, OR SAVING</span>
          <h2>{sources.length} research {sources.length === 1 ? "source" : "sources"} without a supplier</h2>
        </div>
      </div>
      <p className="wb-asset-note">These collected sources carry no vendor, product, price, or quote facts, so they are not ranked, compared, or counted as savings. They stay inspectable with their capture state and missing terms.</p>
      <div className="wb-supplier-list">
        {sources.map((source) => (
          <article className="wb-supplier-row" key={source.id}>
            <div className="wb-vendor-monogram" aria-hidden="true"><Icon name="file" size={16} /></div>
            <div className="wb-supplier-name">
              <h2>{source.sourceKind}</h2>
              <p>Captured {formatDate(source.capturedAt)} · {formatStateLabel(source.completeness)}</p>
              <span>{source.missingFacts.length === 0 ? "Commercial facts not assessed" : `Still missing: ${source.missingFacts.map((fact) => formatStateLabel(fact)).join(", ")}`}</span>
            </div>
            <div className="wb-supplier-status">
              <ProvenancePill mode={source.provenance} ownerAuthoredTerms={source.ownerAuthoredTerms} />
              {source.sourceUrl !== null ? <><a className="wb-paper-source" href={source.sourceUrl} target="_blank" rel="noreferrer" title={source.sourceUrl}><span>Source record · Open original</span><Icon name="link" size={12} /></a><span className="wb-micro">{source.sourceUrl}</span></> : <span className="wb-micro">No public source URL in this projection.</span>}
            </div>
          </article>
        ))}
      </div>
      {truncated ? <p className="wb-card-footnote"><Icon name="warning" size={13} /> More research sources exist than this projection shows.</p> : null}
    </section>
  );
}
...
function jobStateSummary(job: WorkbenchSnapshot["jobs"][number]): string {
  if (job.state === "partial") return "Partial provider outcome: recorded evidence is retained and no additional outcome is recorded in this projection.";
